...
namespace Clean_Reader.SubPages
{
    ...
    /// </summary>
    public sealed partial class TopicDetailPage : RichasyPage
    {
        public ObservableCollection<Book> DisplayCollection = new ObservableCollection<Book>();
        private int index = 1;
        AppViewModel vm = App.VM;
        private SpecialContainer _container;
        private bool _isRequesting = false;
        public TopicDetailPage() : base()
        {
        ...
        protected async override void OnNavigatedTo(NavigationEventArgs e)
        {
            if (e.NavigationMode == NavigationMode.Back)
                return;
            if (e.Parameter != null && e.Parameter is SpecialContainer con)
            {
                if (con != _container)
                {
                    _container = con;
        ...
        {
            MainPage.Current.SetSubtitle(_container.Name);
            if (_isRequesting)
                return;
            _isRequesting = true;
            ...
            var response = await vm._yuenovClient.GetSpecialDetailAsync(_container.Id, index);
